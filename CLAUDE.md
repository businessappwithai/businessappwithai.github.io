...
| `todo.html` | Two groups. **Before 1.0.0**, in priority order: security, the DeepSeek harness and usability, documentation, end-to-end testing, the license audit — the item *in progress* is the second, not the first, and the section subtitle says so rather than implying the list is the work queue. **After it**: a research agent running wholly on the local model. Each item carries what *done* looks like. Linked from every footer's Product column and from both experimental-software notices on the home page. Deliberately **not** in the primary nav: it is at its seven-item ceiling |
